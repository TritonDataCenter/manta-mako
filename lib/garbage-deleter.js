...
function GarbageDeleter(opts) {
    ...
    if (self.metricsManager) {
        self.metrics = {
            badInstructionFiles: self.metricsManager.collector.counter({
                name: 'bad_instruction_files',
                help: 'Counter incremented for each instruction file found to be ' +
                    'invalid'
            }),
            deleteCountErrors: self.metricsManager.collector.counter({
                name: 'delete_count_errors',
            ...
            deleteTimeMinSeconds: self.metricsManager.collector.gauge({
                name: 'delete_time_min_seconds',
                help: 'Gauge of minimum time spent in fs.unlink() deleting a ' +
                    'single Manta object (fastest delete)'
            }),
            deleteTimeTotalSeconds: self.metricsManager.collector.counter({
                name: 'delete_time_total_seconds',
            ...
            instructionFilesDeleted: self.metricsManager.collector.counter({
                name: 'instruction_files_deleted',
            ...
            instructionFilesProcessed: self.metricsManager.collector.counter({
                name: 'instruction_files_processed',
            ...
            instructionLinesProcessed: self.metricsManager.collector.counter({
                name: 'instruction_lines_processed',
                help: 'Counter incremented for each instruction line processed'
            }),
            queuedInstructionFiles: self.metricsManager.collector.gauge({
                name: 'queued_instruction_files',
    ...
    self.addCounter('badInstructionFiles', 0);
    self.addCounter('deleteCountErrors', 0);
    self.addCounter('deleteCountTotal', 0);
    self.setGauge('deleteTimeMaxSeconds', 0);
    self.setGauge('deleteTimeMinSeconds', Infinity);
    self.addCounter('deleteTimeTotalSeconds', 0);
    ...
    self.setGauge('queuedInstructionFiles', 0);
...
GarbageDeleter.prototype.getCounter = function getCounter(counterName) {
    ...
    self.metrics[counterName].getValue();
};
...
GarbageDeleter.prototype.getGauge = function getGauge(gaugeName) {
    ...
    self.metrics[gaugeName].getValue();
};
...
function processFile(instrFile, callback) {
    ...
    function _doneProcessing(err) {
        ...
        // directory.
        if (err) {
            self.addCounter('badInstructionFiles', 1);
    ...
    self.readLines(filename, function _onReadFile(err, lines) {
        if (err) {
            ...
            // XXX what kind of errors can we hit?
            _doneProcessing(err);
            return;
        }
        ...
        forEachParallel({
            ...
            func: function _runInstructions(line, cb) {
                ...
                if (fields.length !== 5) {
                    ...
                    }, 'File "' + instrFile + '" has invalid number' +
                ...
                self.fsUnlink(deleteFile, function _unlinkMantaFile(unlinkErr) {
                    ...
                    self.addCounter('deleteTimeTotalSeconds', elapsed);
                    if (elapsed > self.getGauge('deleteTimeMaxSeconds')) {
                        self.setGauge('deleteTimeMaxSeconds', elapsed);
                    }
                    if (elapsed < self.getGauge('deleteTimeMinSeconds')) {
                    ...
                    if (unlinkErr) {
                        if (unlinkErr.code === 'ENOENT') {
                            ...
                            self.addCounter('deleteCountErrors', 1);
                            return;
                        }
                    } else {
...
GarbageDeleter.prototype.countQueue = function countQueue() {
    ...
    self.fsReaddir(self.instructionDir, function _onReaddir(err, files) {
        if (!err) {
            self.setGauge('queuedInstructionFiles', files.length)
        } else {
            self.log.warn({
