 ...
 * Summary:
 *
 *   This program is intended to be run on mako/storage zones in Manta and takes
 *   an instructions file (in the format generated by manta-garbage collector)
 *   and a storageId as arguments and then processes the instructions file,
 *   deleting files from /manta where possible.
 *
 *   It will write diagnostic information to stderr while it is operating and
 *   when it is complete it will write 6 integers to stdout separated by spaces.
 *   These values are:
 *
...
fn main() -> io::Result<()> {
    ...
    /*
     * It is likely that we will encounter a path to an instructions file that
     * does not exist. In these cases we need to bail out of the program and
     * let the wrapping shell script delete the file.
     */
    ...
    for line in instructions.lines() {
        ...
        /*
         * Due to a bug in manta-garbage-collector it is possible that we will
         * encounter invalid instructions. If we do encounter an invalid
         * instruction it is important that we preserve the original instruction
         * file for postmortem analysis. Here we check that the line has the
         * correct number of values and if not we log the invalid line, set
         * invalid_instruction_seen to true and skip to the next instruction.
         */
        ...
        /*
         * Sometimes we will encounter a situation where the object to be
         * deleted has already been deleted. In these cases we want to output
         * that this happened and move forward without doing anything more
    ...
    /*
     * If we encounter an invalid instruction, exit with code 1 which signals to
     * the caller the current instruction file should be preserved for later
     * analysis.
     */
